...
resource "aws_s3_bucket" "static_website" {
  bucket = "franchise-api-${random_id.bucket_suffix.hex}"
  # Quitar acl también porque está deprecado y genera conflicto
}
...
resource "aws_s3_object" "index_html" {
  bucket       = aws_s3_bucket.static_website.id
  key          = "index.html"
  source       = "../src/doc/index.html"
  content_type = "text/html"
  # No acl aquí
}

resource "aws_s3_object" "swagger_yaml" {
  bucket       = aws_s3_bucket.static_website.id
  key          = "swagger.yaml"
  source       = "../src/doc/swagger.yaml"
